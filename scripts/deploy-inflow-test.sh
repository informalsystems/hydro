    ...
    cat > "$CONFIG_FILE" <<EOF
{
  ...
  "mars_params": "neutron1x4rgd7ry23v2n49y7xdzje0743c5tgrnqrqsvwyya2h6m48tz4jqqex06x",
  "mars_red_bank": "neutron1n97wnm7q6d2hrcna3rqlnyqw2we6k0l8uqvmyqq6gsml92epdu7quugyph",
  "token_metadata": {
...
MARS_CREDIT_MANAGER=$(jq -r '.mars_credit_manager' $CONFIG_FILE)
MARS_PARAMS=$(jq -r '.mars_params' $CONFIG_FILE)
MARS_RED_BANK=$(jq -r '.mars_red_bank' $CONFIG_FILE)
...
instantiate_control_center() {
    ...
    if [ -n "$EXISTING" ] && [ "$EXISTING" != "null" ]; then
        echo -e "${YELLOW}Existing Control Center address: $EXISTING${NC}"
...
instantiate_vault() {
    ...
    if [ -n "$EXISTING" ] && [ "$EXISTING" != "null" ]; then
        echo -e "${YELLOW}Existing Vault address: $EXISTING${NC}"
...
instantiate_mars_adapter() {
    EXISTING=$(jq -r '.contracts.mars_adapter // empty' $CONFIG_FILE)
    if [ -n "$EXISTING" ] && [ "$EXISTING" != "null" ]; then
        echo -e "${YELLOW}Existing Mars Adapter address: $EXISTING${NC}"
        read -p "Do you want to reinstantiate Mars Adapter? (y/N): " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            MARS_ADAPTER_ADDRESS="$EXISTING"
...
instantiate_ibc_adapter() {
    EXISTING=$(jq -r '.contracts.ibc_adapter // empty' $CONFIG_FILE)
    if [ -n "$EXISTING" ] && [ "$EXISTING" != "null" ]; then
        echo -e "${YELLOW}Existing IBC Adapter address: $EXISTING${NC}"
        read -p "Do you want to reinstantiate IBC Adapter? (y/N): " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            IBC_ADAPTER_ADDRESS="$EXISTING"
...
register_noble_chain_ibc_adapter() {
...
EOF
    ...
    $NEUTRON_CLI tx wasm execute "$IBC_ADAPTER_ADDRESS" "$REGISTER_CHAIN_MSG" \
        --from "$DEPLOYER_WALLET" \
        $NEUTRON_TX_FLAGS \
        --output json &> ./register_noble_chain_ibc_adapter_res.json

    TX_HASH=$(grep -o '{.*}' ./register_noble_chain_ibc_adapter_res.json | jq -r '.txhash')
    retry_command "$NEUTRON_CLI q tx $TX_HASH $NEUTRON_NODE_FLAG --output json" 60 > /dev/null

    echo "Noble chain registered on IBC Adapter"
}

register_usdc_token_ibc_adapter() {
...
EOF
    ...
    $NEUTRON_CLI tx wasm execute "$IBC_ADAPTER_ADDRESS" "$REGISTER_TOKEN_MSG" \
        --from "$DEPLOYER_WALLET" \
        $NEUTRON_TX_FLAGS \
        --output json &> ./register_usdc_token_ibc_adapter_res.json

    TX_HASH=$(grep -o '{.*}' ./register_usdc_token_ibc_adapter_res.json | jq -r '.txhash')
    retry_command "$NEUTRON_CLI q tx $TX_HASH $NEUTRON_NODE_FLAG --output json" 60 > /dev/null
...
unregister_adapter() {
    ...
    printf "Unregistering $adapter_name from Vault"

    UNREGISTER_MSG=$(cat <<EOF
{
  "unregister_adapter": {
    "name": "$adapter_name"
  }
...
EOF
    ...
    # Use || true to not fail if adapter is not registered
    $NEUTRON_CLI tx wasm execute "$VAULT_ADDRESS" "$UNREGISTER_MSG" \
        --from "$DEPLOYER_WALLET" \
        $NEUTRON_TX_FLAGS \
        --output json &> ./unregister_adapter_res.json || true

    if [ -f "./unregister_adapter_res.json" ]; then
        TX_HASH=$(grep -o '{.*}' "./unregister_adapter_res.json" | jq -r '.txhash' 2>/dev/null || echo "")
        if [ -n "$TX_HASH" ] && [ "$TX_HASH" != "null" ]; then
            retry_command "$NEUTRON_CLI q tx $TX_HASH $NEUTRON_NODE_FLAG --output json" 60 > /dev/null || true
            echo "$adapter_name unregistered from Vault"
        else
            echo "$adapter_name was not registered (skipped)"
        fi
    else
        echo "$adapter_name was not registered (skipped)"
    fi
...
main() {
    # Initialize instantiation tracking variables
    CONTROL_CENTER_NEWLY_INSTANTIATED=false
    VAULT_NEWLY_INSTANTIATED=false

    # Step 1: Deploy Contract Code (Upload WASMs)
    echo -e "${BLUE}=== Step 1: Deploy Contract Code ===${NC}"
    echo ""
    deploy_control_center_code
    deploy_vault_code
    deploy_mars_adapter_code
    ...
    echo -e "${GREEN}=== Step 2: Instantiate Contracts ===${NC}"
    echo ""
    instantiate_control_center
    echo ""
    instantiate_vault
    ...
    # Step 3: Register Vault in Control Center (only if newly instantiated)
    if [ "$CONTROL_CENTER_NEWLY_INSTANTIATED" = true ] || [ "$VAULT_NEWLY_INSTANTIATED" = true ]; then
        echo -e "${GREEN}=== Step 3: Register Vault in Control Center ===${NC}"
        echo ""
        register_subvault
        echo ""
    else
        echo -e "${YELLOW}=== Step 3: Skipping Subvault Registration (already registered) ===${NC}"
        echo ""
    fi

    # Step 4: Register Adapters on Vault
    echo -e "${GREEN}=== Step 4: Register Adapters on Vault ===${NC}"
    echo ""

    # If vault wasn't newly instantiated, unregister adapters first to avoid errors
    if [ "$VAULT_NEWLY_INSTANTIATED" = false ]; then
        echo "Unregistering existing adapters first..."
        unregister_adapter "Mars Adapter"
        echo ""
        unregister_adapter "IBC Adapter"
        echo ""
    fi
