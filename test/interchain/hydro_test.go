...
func (s *HydroSuite) TestValidatorSlashing() {
	...
	// lockTxData tokens
	lockAmount := "300"
	powerAfterSlashing := 271 // todo: change to do after contract is fix to to floor rounding
...
func (s *HydroSuite) GetUserVotingPower(contractAddr string, address string) int64 {
	userPowerQueryData := map[string]interface{}{
		"user_voting_power": map[string]interface{}{
			"address": address,
		},
	}
	userPowerQueryJson, err := json.Marshal(userPowerQueryData)
	s.Require().NoError(err)

	response, _, err := s.NeutronChain.Validators[0].ExecQuery(
		s.GetContext(),
		"wasm", "contract-state", "smart", contractAddr, string(userPowerQueryJson),
	)
	s.Require().NoError(err)
