...
#[cw_serde]
pub struct ConfigV2_0_1 {
    pub hydro_contract: Addr,
}

#[cw_serde]
pub struct TributeV2_0_1 {
    pub round_id: u64,
    pub tranche_id: u64,
    pub proposal_id: u64,
    pub tribute_id: u64,
...
// Migrating from 1.1.1 to 2.0.1 will:
...
pub fn migrate_v1_1_1_to_v2_0_1(deps: &mut DepsMut) -> Result<(), ContractError> {
...
fn migrate_config(storage: &mut dyn Storage) -> Result<ConfigV2_0_1, ContractError> {
...
fn migrate_tributes(deps: &mut DepsMut, config: ConfigV2_0_1) -> Result<(), ContractError> {
    ...
    for old_tribute in OLD_ID_TO_TRIBUTE_MAP.range(deps.storage, None, None, Order::Ascending) {
        ...
        let new_tribute = TributeV2_0_1::from(old_tribute, &constants_resp.constants)?;
