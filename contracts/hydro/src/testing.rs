...
use crate::state::{
    LockEntry, RoundLockPowerSchedule, Vote, CONSTANTS, TOKEN_INFO_PROVIDERS, USER_LOCKS,
    VOTE_MAP_V2,
};
        ...
        if let Some((contract_address, _)) = lsm_provider.clone() {
            TOKEN_INFO_PROVIDERS
                .save(
                    &mut deps.storage,
                    contract_address.to_string(),
                    &TokenInfoProvider::LSMHub(TokenInfoProviderLSMHub {
                        contract: contract_address.to_string(),
                        cache: HashMap::new(),
                    }),
                )
...
// Test case:
//      1. User votes with 1-round-long-lock for proposal with deployment_duration = 1
//      2. User unvotes with the same lock (old vote gets deleted)
//      3. User votes for proposal from step #1 again
//         (or any other with deployment_duration that it should be allowed to vote)
#[test]
fn unvote_and_revote_test() {
    ...
    let lockup_info = &res.lockups_with_per_tranche_infos[0];
    let tranche_info = lockup_info
        .per_tranche_info
        .iter()
    ...
    assert_eq!(
        Some(proposal_id),
        tranche_info.current_voted_on_proposal,
        "Lock should be voting on proposal after initial vote"
    );
    assert_eq!(
        current_round_id, tranche_info.next_round_lockup_can_vote,
        "Next voting round should be current_round when lock is currently voting"
    );
    ...
    let tranche_info = lockup_info
        ...
        .unwrap();
    assert_eq!(
        None, tranche_info.current_voted_on_proposal,
        "Lock should not be voting after unvote"
    );
    assert_eq!(
        current_round_id, tranche_info.next_round_lockup_can_vote,
        "Lock should be able to vote again in current round after unvote"
    );

    // Verify proposal power was reduced to zero
    let proposal_after_unvote =
        query_proposal(deps.as_ref(), current_round_id, tranche_id, proposal_id)
            .unwrap()
            .proposal;
    assert_eq!(Uint128::zero(), proposal_after_unvote.power);
    ...
    assert_eq!(
        Some(proposal_id),
        tranche_info.current_voted_on_proposal,
        "Lock should be voting on proposal after revote"
    );
    assert_eq!(
        current_round_id, tranche_info.next_round_lockup_can_vote,
        "Next voting round should be current_round when lock is currently voting"
    ...
    // Verify proposal power was restored
    let final_proposal = query_proposal(deps.as_ref(), current_round_id, tranche_id, proposal_id)
        .unwrap()
        .proposal;
    assert_eq!(info.funds[0].amount.u128(), final_proposal.power.u128());
}
...
fn unvote_forbidden_locks() {
    ...
    let other_user_token = Coin::new(1000u64, VALIDATOR_1_LST_DENOM_1.to_string());
...
fn multi_tranches_test() {
    ...
    let info2 = get_message_info(
        &deps.api,
        "addr0001",
        &[Coin::new(2000u64, VALIDATOR_1_LST_DENOM_1.to_string())],
    );
    let msg = ExecuteMsg::LockTokens {
        lock_duration: ONE_MONTH_IN_NANO_SECONDS,
        proof: None,
...
fn total_voting_power_tracking_test() {
    ...
    let info1 = get_message_info(
        &deps.api,
        user_address,
        &[Coin::new(10u64, VALIDATOR_1_LST_DENOM_1.to_string())],
    );
    let msg = ExecuteMsg::LockTokens {
        lock_duration: ONE_MONTH_IN_NANO_SECONDS,
        proof: None,
    ...
    let info2 = get_message_info(
        &deps.api,
        user_address,
        &[Coin::new(20u64, VALIDATOR_1_LST_DENOM_1.to_string())],
    );
    let msg = ExecuteMsg::LockTokens {
    ...
    let info2 = get_message_info(
        &deps.api,
        user_address,
        &[Coin::new(50u64, VALIDATOR_1_LST_DENOM_1.to_string())],
    );
    let msg = ExecuteMsg::LockTokens {
        lock_duration: THREE_MONTHS_IN_NANO_SECONDS,
        proof: None,
...
proptest! {
    ...
    fn relock_proptest(old_lock_remaining_time: u64, new_lock_duration: u8) {
        ...
        let info = get_message_info(&deps.api, "addr0001", &[Coin::new(1000u64, VALIDATOR_1_LST_DENOM_1.to_string())]);
        let msg = get_default_instantiate_msg(&deps.api);
...
fn contract_pausing_test() {
    ...
    let msgs = vec![
        ...
        ExecuteMsg::AddTokenInfoProvider {
            token_info_provider: TokenInfoProviderInstantiateMsg::LSMHub {
                code_id: 0,
                msg: Binary::default(),
                label: String::new(),
                admin: None,
