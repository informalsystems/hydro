...
pub enum ContractError {
    ...
    #[error("Insufficient balance for transfer. Has: {has}, needs: {needs}")]
    ...
    #[error("Invalid Cosmos address: {address} - {reason}")]
    InvalidCosmosAddress { address: String, reason: String },
    ...
    #[error("Recover address not allowed: {address}")]
    RecoverAddressNotAllowed { address: String },

    #[error("Recover address already exists: {address}")]
    RecoverAddressAlreadyExists { address: String },

    #[error("Recover address doesn't exist: {address}")]
    RecoverAddressDoesNotExist { address: String },

    #[error("Eureka fee too low: {fee}, minimum is {min}")]
    EurekaFeeTooLow { fee: Uint128, min: Uint128 },

    #[error("Eureka fee too high: {fee}, maximum is {max}")]
    EurekaFeeTooHigh { fee: Uint128, max: Uint128 },
}
