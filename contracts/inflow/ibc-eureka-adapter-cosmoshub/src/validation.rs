...
use crate::state::{
    Depositor, ADMINS, ALLOWED_DESTINATION_ADDRESSES, EXECUTORS, WHITELISTED_DEPOSITORS,
};
...
pub fn validate_executor_caller(deps: &DepsMut, info: &MessageInfo) -> Result<(), ContractError> {
...
pub fn get_depositor(deps: Deps, depositor_address: String) -> StdResult<Depositor> {
...
/// Get destination address from storage, normalizing the address first (i.e. strip 0x prefix if present, convert to lowercase).
/// Returns the normalized address string if it exists in the allowlist.
pub fn get_destination_address(deps: &Deps, address: &str) -> Result<String, ContractError> {
    let normalized_address = normalize_evm_address(address)?;

    ALLOWED_DESTINATION_ADDRESSES
        .may_load(deps.storage, normalized_address.clone())?
