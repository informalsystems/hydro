...
# Arc testnet deployment script for ReserveAdapter (UUPS upgradeable proxy).
# Wraps script/DeployReserveAdapter.s.sol; see that file for full details.
...
if [[ -z "$VAULT_ADDRESS" ]]; then
  ...
  echo "Usage: VAULT_ADDRESS=0x... bash bin/06_deploy_reserve_adapter.sh"
  exit 1
fi
echo "Vault:    $VAULT_ADDRESS"

# ── Export env vars expected by DeployReserveAdapter.s.sol ───────────────────
