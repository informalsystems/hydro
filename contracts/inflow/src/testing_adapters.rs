...
use cosmwasm_std::{testing::mock_env, Addr, CosmosMsg, Uint128, WasmMsg};
...
use adapter_interface::msg::{AdapterQueryMsg, AvailableAmountResponse, InflowDepositResponse};
use cosmwasm_std::{
    from_json,
...
struct MockAdapterConfig {
    available_for_deposit: Uint128,
    available_for_withdraw: Uint128,
    current_deposit: Uint128,
    should_fail_queries: bool,
}

impl MockAdapterConfig {
    fn new(deposit_capacity: u128, withdraw_capacity: u128, current_deposit: u128) -> Self {
        Self {
...
/// Creates mock dependencies with custom adapter query responses
fn mock_dependencies_with_adapters(
    adapter_configs: HashMap<String, MockAdapterConfig>,
) -> OwnedDeps<MockStorage, MockApi, MockQuerier, neutron_sdk::bindings::query::NeutronQuery> {
    let mut deps = mock_dependencies();

    deps.querier.update_wasm(move |query| match query {
        WasmQuery::Smart { contract_addr, msg } => {
            // Look up adapter config by contract address
            let adapter_config = adapter_configs.get(contract_addr);
            ...
            if let Some(config) = adapter_config {
                ...
                // Try to parse as adapter query
                let parsed: Result<AdapterQueryMsg, _> = from_json(msg);
                if let Ok(adapter_query) = parsed {
                    match adapter_query {
                        AdapterQueryMsg::AvailableForDeposit { .. } => {
                            let response = AvailableAmountResponse {
                        ...
                        AdapterQueryMsg::InflowDeposit { .. } => {
                            let response = InflowDepositResponse {
...
fn test_deposit_with_single_active_adapter() {
    ...
    deps = mock_dependencies_with_adapters(adapter_configs);
    ...
    match &res.messages[0].msg {
        CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            let adapter_msg: adapter_interface::msg::AdapterExecuteMsg = from_json(msg).unwrap();
            assert!(matches!(
                adapter_msg,
                adapter_interface::msg::AdapterExecuteMsg::Deposit {}
            ));
        }
...
fn test_deposit_no_adapters_stays_in_contract() {
    ...
    let deps_no_adapters = HashMap::new();
    deps = mock_dependencies_with_adapters(deps_no_adapters);
    let mut env = mock_env();
...
fn test_deposit_with_failing_adapter_stays_in_contract() {
    ...
    let mut adapter_configs = HashMap::new();
    adapter_configs.insert(adapter_addr.to_string(), MockAdapterConfig::failing());
    deps = mock_dependencies_with_adapters(adapter_configs);
    let mut env = mock_env();
...
fn test_deposit_skips_failing_adapter() {
    ...
    let mut adapter_configs = HashMap::new();
    adapter_configs.insert(
        failing_adapter_addr.to_string(),
        MockAdapterConfig::failing(),
    );
    adapter_configs.insert(
        working_adapter_addr.to_string(),
        MockAdapterConfig::new(10000, 0, 0),
    );
    deps = mock_dependencies_with_adapters(adapter_configs);
    let mut env = mock_env();
...
fn test_withdraw_partial_fulfillment_with_queue() {
    ...
    // Simulate deployed amount of 5000 tokens (e.g., deployed to a multisig for yield farming)
    use crate::state::DEPLOYED_AMOUNT;
    DEPLOYED_AMOUNT
        .save(deps.as_mut().storage, &Uint128::new(5000), env.block.height)
        .unwrap();
    ...
    let res = execute(deps.as_mut(), env, info, ExecuteMsg::Withdraw {}).unwrap();
    ...
    match &res.messages[0].msg {
        CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            let adapter_msg: adapter_interface::msg::AdapterExecuteMsg = from_json(msg).unwrap();
            match adapter_msg {
                adapter_interface::msg::AdapterExecuteMsg::Withdraw { coin } => {
                    assert_eq!(coin.denom, DEPOSIT_DENOM);
                    assert_eq!(coin.amount, Uint128::new(3000));
                }
                _ => panic!("Expected AdapterExecuteMsg::Withdraw"),
            }
        }
    ...
    assert_eq!(res.attributes[2].value, "2000"); // vault_shares_sent
