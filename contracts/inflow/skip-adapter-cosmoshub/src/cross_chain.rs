use cosmwasm_std::{Coin, CosmosMsg, Env, IbcMsg, IbcTimeout, StdResult, Timestamp, Uint128};
use serde_json::json;
...
/// Build IBC transfer message for cross-chain swap with forward path.
/// On Cosmos Hub, contracts issue IbcMsg::Transfer directly — no intermediary needed.
pub fn build_cross_chain_swap_ibc_msg(
    coin: Coin,
    ...
    let memo = if forward_path.len() > 1 {
        ...
        // Single hop (direct to swap chain): use the wasm hook as memo
        wasm_hook_memo
    };

    Ok(CosmosMsg::Ibc(IbcMsg::Transfer {
        channel_id: first_hop.channel.clone(),
        to_address: first_hop.receiver.clone(),
        amount: coin,
        timeout: IbcTimeout::with_timestamp(Timestamp::from_nanos(timeout_nanos)),
        memo: Some(memo),
    }))
}
