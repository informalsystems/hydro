...
pub fn validate_route_config(route: &UnifiedRoute) -> Result<(), ContractError> {
    ...
    if route.venue.is_local() {
        ...
        for (idx, hop) in route.return_path.iter().enumerate() {
            if hop.chain_id.is_empty() {
                return Err(ContractError::InvalidReturnPath {
                    reason: format!("Return hop {} has empty chain_id", idx),
                });
            }
            if hop.channel.is_empty() {
                return Err(ContractError::InvalidReturnPath {
                    reason: format!("Return hop {} has empty channel", idx),
                });
            }
        }

        if route.recover_address.as_deref().unwrap_or("").is_empty() {
            return Err(ContractError::InvalidRoute {
                reason: "Cross-chain routes must have recover_address set".to_string(),
            });
        }
    }
