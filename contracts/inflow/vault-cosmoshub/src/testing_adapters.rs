...
fn mock_dependencies_with_adapters(
    ...
    OwnedDeps<MockStorage, MockApi, MockQuerier, token_bindings::TokenFactoryQuery>,
    MockWasmQuerier,
) {
    let mut deps = mock_dependencies();
...
fn setup_contract_with_vault_denom(
    deps: &mut OwnedDeps<MockStorage, MockApi, MockQuerier, token_bindings::TokenFactoryQuery>,
...
fn test_deposit_with_single_adapter_no_auto_allocation() {
    ...
    match &res.messages[0].msg {
        CosmosMsg::Custom(token_bindings::TokenFactoryMsg::MintTokens {
            denom,
            amount,
            mint_to_address: _,
        }) => {
...
fn test_withdraw_all_or_nothing_can_fulfill() {
    ...
    match &res.messages[2].msg {
        CosmosMsg::Custom(token_bindings::TokenFactoryMsg::BurnTokens {
            denom,
            amount,
            burn_from_address: _,
        }) => {
...
fn test_deposit_to_adapter_success() {
    ...
    assert_eq!(res.attributes[4].value, "5000"); // amount_deposited
    assert_eq!(res.attributes[5].value, "NotTracked");
