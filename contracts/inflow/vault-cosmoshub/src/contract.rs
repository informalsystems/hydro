...
use token_bindings::{DenomUnit, FullDenomResponse, Metadata, TokenFactoryMsg, TokenFactoryQuery};
...
use crate::{
    ...
    state::{
        ...
        ADAPTERS, CONFIG, LAST_FUNDED_WITHDRAWAL_ID, MIGRATION_MINTED, NEXT_PAYOUT_ID,
        NEXT_WITHDRAWAL_ID, PAUSED, PAYOUTS_HISTORY, USER_WITHDRAWAL_REQUESTS, WHITELIST,
...
fn deposit(
    ...
    let mint_vault_shares_msg = TokenFactoryMsg::MintTokens {
        denom: config.vault_shares_denom.clone(),
        amount: vault_shares_to_mint,
...
fn withdraw(
    ...
    let burn_shares_msg = TokenFactoryMsg::BurnTokens {
        denom: vault_shares_denom.clone(),
        amount: vault_shares_sent,
...
fn mint_for_migration(
    ...
    let mint_msg = CosmosMsg::Custom(TokenFactoryMsg::MintTokens {
        denom: config.vault_shares_denom.clone(),
        amount: shares_to_mint,
        mint_to_address: conversion_addr.to_string(),
    });

    let mut response = Response::new()
        .add_message(mint_msg)
        .add_attribute("action", "mint_for_migration")
        .add_attribute("shares_minted", shares_to_mint)
        .add_attribute("conversion_contract", conversion_addr);

    if let Some(amount) = deployed_amount {
        let update_msg = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: config.control_center_contract.to_string(),
            msg: to_json_binary(&ControlCenterExecuteMsg::UpdateDeployedAmount {
                amount,
        ...
        response = response
            ...
            .add_attribute("deployed_amount", amount);
    }
...
fn get_token_ratio_to_base_token(
    deps: &Deps<TokenFactoryQuery>,
    config: &Config,
) -> StdResult<Decimal> {
...
fn query_shares_issued(deps: &Deps<TokenFactoryQuery>) -> StdResult<Uint128> {
    Ok(deps
        .querier
...
fn query_dry_run_deposit(deps: &Deps<TokenFactoryQuery>, amount: Uint128) -> StdResult<Uint128> {
    let config = load_config(deps.storage)?;
    let pool_info = get_control_center_pool_info(deps, &config.control_center_contract)?;
...
pub fn query_available_for_deployment(
    deps: &Deps<TokenFactoryQuery>,
    env: &Env,
) -> StdResult<Uint128> {
    let config = load_config(deps.storage)?;
...
fn query_whitelist(deps: &Deps<TokenFactoryQuery>) -> StdResult<WhitelistResponse> {
...
fn query_list_adapters(deps: &Deps<TokenFactoryQuery>) -> StdResult<AdaptersListResponse> {
...
pub fn reply(
    ...
    match reply_paylod {
        ReplyPayload::CreateDenom { subdenom, metadata } => {
            ...
            let full_denom: FullDenomResponse = deps.querier.query(
                &cosmwasm_std::QueryRequest::Custom(TokenFactoryQuery::FullDenom {
                    creator_addr: env.contract.address.to_string(),
                    subdenom,
                }),
            )?;
            ...
            let msg = create_set_denom_metadata_msg(full_denom.denom.clone(), metadata);
...
/// Creates a SetMetadata message to set the metadata for the previously created `full_denom` token.
fn create_set_denom_metadata_msg(
    full_denom: String,
    token_metadata: DenomMetadata,
) -> CosmosMsg<TokenFactoryMsg> {
    CosmosMsg::Custom(TokenFactoryMsg::SetMetadata {
        ...
        metadata: Metadata {
            ...
            display: Some(token_metadata.display),
            name: Some(token_metadata.name),
            symbol: Some(token_metadata.symbol),
        },
    })
}
