...
pub fn tick_to_sqrt_price(tick_index: i64) -> Result<BigDecimal, ContractError> {
    ...
    Err(ContractError::InvalidConversion {})
}
...
pub fn price_to_tick(price: &BigDecimal) -> Result<i64, String> {
    ...
    if price > &BigDecimal::from(1u32) {
        let mut idx = 0;
        while let Some(data) = TICK_EXP_CACHE.get(&idx) {
...
pub fn calc_liquidity_amount1(
    ...
    let (sqrt_price_a, sqrt_price_b) = match current_sqrt_price.cmp(&lower_tick_sqrt) {
        Ordering::Equal => {
            // If sqrtPriceA equals sqrtPriceB, return zero liquidity
            return BigDecimal::zero();
        }
        Ordering::Greater => (lower_tick_sqrt.clone(), current_sqrt_price),
        Ordering::Less => (current_sqrt_price, lower_tick_sqrt.clone()),
    };
...
pub fn calc_liquidity_amount0(
    ...
    let numerator = amount0 * &sqrt_price_a * &sqrt_price_b;
    let denominator = &sqrt_price_b - &sqrt_price_a;
